...
userRouter.post("/shops-available", async (req, res) : Promise <any> => {
  const { area, pincode } = req.body;
...
userRouter.get("/advertisements", async (req, res) : Promise <any> => {
  try {
    const advertisements = await prisma.adver.findMany({
      include: {
        shop: {
          select: {
            id: true,
...
userRouter.get("/shop/:shopId/products", async (req, res) : Promise <any> => {
...
userRouter.post("/advertisement/:adId/view", async (req, res) : Promise <any> => {
...
userRouter.post("/advertisement/:adId/click", async (req, res) : Promise <any>=> {
  const { adId } = req.params;
...
userRouter.post("/cart/add", async (req, res): Promise<any> => {
...
userRouter.post("/cart/items", async (req, res): Promise<any> => {
...
userRouter.delete("/cart/remove/:cartItemId", async (req, res): Promise<any> => {
...
userRouter.put("/cart/update", async (req, res) : Promise<any> => {
...
// Get user coins
userRouter.post("/coins", async (req, res) : Promise<any> => {
  ...
  try {
    ...
    return res.status(200).json({ success: true, coins: user?.coinsAvailable || 0 });
  } catch (e) {
...
userRouter.get("/shop/:shopId", async (req, res)  : Promise <any> => {
  const { shopId } = req.params;
...
userRouter.get("/cart/:userId", async (req, res): Promise<any> => {
...
userRouter.delete("/cart/clear/:userId", async (req, res): Promise<any> => {
  const { userId } = req.params;
...
userRouter.delete("/cart/clear-shop/:userId/:shopId", async (req, res): Promise<any> => {
...
userRouter.post("/cart/add-with-conflict-check", async (req, res): Promise<any> => {
...
userRouter.post("/cart/bulk-add", async (req, res): Promise<any> => {
  ...
  try {
    await prisma.$transaction(async (tx) => {
      ...
      for (const orderData of orders) {
        ...
        const getAvailableCoins = async (userId: number, shopId: number) => {
          // Get coins received from shop
          const coinsReceived = await tx.shopToUserCoin.aggregate({
            where: {
              userId: userId,
              shopId: shopId
            },
          ...
          const coinsUsed = await tx.userToShopCoin.aggregate({
            where: {
              userId: userId,
              shopId: shopId
            },
          ...
          const totalReceived = parseInt(coinsReceived._sum.volume as any) || 0;
          const totalUsed = parseInt(coinsUsed._sum.volume as any) || 0;
        ...
        // Validate and handle coin usage for payment (not offers)
        if (orderData.coinsUsed > 0) {
          const availableCoins = await getAvailableCoins(parseInt(userId), orderData.shopId);
        ...
        if (orderData.coinsUsed > 0) {
          await tx.userToShopCoin.create({
            data: {
              userId: parseInt(userId),
              shopId: orderData.shopId,
    ...
    return res.status(500).json({
      success: false,
      message: (e as Error).message || "Error placing order"
    });
  }
});
...
userRouter.post("/orders", async (req, res) : Promise <any> => {
  ...
  try {
    ...
    const transformedOrders = await Promise.all(orderGroups.map(async (orderGroup) => {
...
userRouter.post('/orders/:orderGroupId/cancel', async (req, res): Promise<any> => {
  try {
    ...
    const result = await prisma.$transaction(
      async (tx) => {
        ...
        if (coinsToRefund > 0) {
          ...
          await tx.shopToUserCoin.create({
            data: {
        ...
        const productUpdates = orderGroup.orders.map(order => 
          tx.product.update({
            where: {
              id: order.productId
            },
...
userRouter.post("/orders/:orderGroupId", async (req, res): Promise<any> => {
    try {
        const { orderGroupId } = req.params;
        const { userId } = req.body;
        ...
        if (productOfferIds.size > 0) {
            freeProducts = await prisma.product.findMany({
                where: {
                    id: {
                         //@ts-ignore
...
userRouter.post("/orders/:orderId", async (req, res) : Promise<any> => {
  ...
  try {
    ...
    const processedItems = await Promise.all(orderGroup.orders.map(async (order) => {
      const itemTotal = order.unitPrice * order.quantity;
...
async function calculateOrderTotal(orders:any, coinsUsed:any, shopCoinValue:any) {
  let originalAmount = 0;
  let totalOfferDiscount = 0;
...
userRouter.post("/getaddress", async (req, res) : Promise <any>=>{
  const {userId} = req.body;
...
userRouter.post("/wallet-transactions", async (req, res): Promise<any> => {
  try {
    const { userId } = req.body;
    ...
    const incomingTransactions = await prisma.shopToUserCoin.findMany({
    ...
    const outgoingTransactions = await prisma.userToShopCoin.findMany({
      where: { userId: userIdNumber },
      include: {
        shop: {
          select: {
...
userRouter.post("/profile", async (req, res): Promise<any> => {
  try {
    const { userId } = req.body;
...
userRouter.post("/update-profile", async (req, res): Promise<any> => {
  try {
...
userRouter.post("/coin-history", async (req, res): Promise<any> => {
  try {
    const { userId } = req.body;
    ...
    const [coinsSent, coinsReceived] = await Promise.all([
      prisma.userToShopCoin.findMany({
      ...
      prisma.shopToUserCoin.findMany({
        where: { userId: userIdInt },
        include: {
          shop: {
            select: {
...
userRouter.post("/shop-coins", async (req, res): Promise<any> => {
  ...
  try {
    ...
    for (const shopId of Object.keys(shopCoins)) {
      const coinsUsed = await prisma.userToShopCoin.aggregate({
        where: {
          userId: parseInt(userId),
          shopId: parseInt(shopId)
        },
      ...
      const used = parseInt(coinsUsed._sum.volume as any) || 0;
         //@ts-ignore
